  ...
  "src/tts/cloud/providers/volcano.js"() {
    ...
    VolcanoProvider = class {
      ...
      async synthesize(params) {
        ...
        const segs = pauseControl && format === "pcm" ? splitForPauses(params.text, {
...
import { existsSync, readFileSync, writeFileSync as writeFileSync2, unlinkSync as unlinkSync2 } from "node:fs";
import { tmpdir as tmpdir2 } from "node:os";
import { dirname, join as join2, resolve } from "node:path";
import { fileURLToPath } from "node:url";
function getAssetsDir() {
  const moduleDir = dirname(fileURLToPath(import.meta.url));
  if (existsSync(join2(moduleDir, "..", "assets"))) return join2(moduleDir, "..", "assets");
  if (existsSync(join2(moduleDir, "..", "..", "assets"))) return join2(moduleDir, "..", "..", "assets");
  return join2(moduleDir, "..", "assets");
}
...
function amplifyWav(srcPath, gain) {
  ...
  const tmp = join2(tmpdir2(), `dsh-voice-amp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.wav`);
  try {
    writeFileSync2(tmp, buf);
    return tmp;
  } catch {
    return srcPath;
  }
...
async function playNotificationSound(sound) {
  ...
  if (BUILTIN_PRESETS.includes(soundName)) {
    const candidate = join2(getAssetsDir(), `${soundName}.wav`);
...
var DEFAULT_CONFIG_PATH = join3(homedir(), ".dsh", "voice", "config.json");
...
<header>
  <h1>dsh \u8BED\u97F3</h1>
  <span class="version-badge"><b>DSH\u8BED\u97F3\u52A9\u624B</b><span class="tag">v${"0.2.3"}</span></span>
...
var PACKAGE_ROOT = join4(dirname2(fileURLToPath2(import.meta.url)), "..");
var PS1 = join4(PACKAGE_ROOT, "notify.ps1");
var IDLE_PS1 = join4(PACKAGE_ROOT, "idle.ps1");
...
var CONFIG_FILE = join4(homeDir, ".dsh", "voice", "config.json");
    ...
    handler: async (req, res) => {
      try {
        if (req.method === "GET") {
          ...
          sendJson(res, 200, { config, version: "0.2.3", engine: activeEngine });
          return;
        }
        if (req.method === "POST") {
          const body = await readBody(req).catch(() => ({}));
          ...
          const { writeFileSync: writeFileSync3, mkdirSync } = await import("node:fs");
          ...
          mkdirSync(join4(CONFIG_FILE, ".."), { recursive: true });
          writeFileSync3(CONFIG_FILE, JSON.stringify(next, null, 2) + "\n", "utf8");
          resetConfigCache();
          await refreshEngine();
          sendJson(res, 200, { ok: true, config: loadConfig() });
          return;
