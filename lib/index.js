...
function httpsJsonPost(apiKey, body, timeoutMs, apiBase) {
  return new Promise((resolve2, reject) => {
    ...
    const requestFn = isHttp ? httpRequest : httpsRequest2;
    const req = requestFn({
      hostname,
...
function concatSegments(segments) {
  ...
  for (const seg of segments) {
    pieces.push(seg.pcm);
    if (seg.pauseAfterMs) pieces.push(silenceBytes2(seg.pauseAfterMs));
  }
  return pcmToWav2(Buffer.concat(pieces));
}
  ...
  "src/tts/cloud/providers/mimo.js"() {
    ...
    MimoProvider = class {
      ...
      async speak(text, options = {}, onBeforePlay) {
        ...
        const tempFile = join2(tmpdir2(), `dsh-voice-mimo-${Date.now()}.wav`);
        this.tempFile = tempFile;
      ...
      async synthesize(params) {
        ...
        const segs = pauseControl ? splitForPauses2(params.text, {
          sentenceMs: this.config.pauseSentenceMs ?? 400,
          commaMs: this.config.pauseCommaMs ?? 200
        }) : null;
        if (segs) {
...
var DEFAULT_CONFIG = {
  engine: "mimo",
  // 默认引擎：小米 MiMo（未配 MiMo Key 时 auto 兜底回退火山/SAPI）
  cloud: {
    provider: "volcano",
